...
function Tip({ active, label, payload }: TipProps) {
  ...
  return (
    <div className="chart-tip">
      ...
      <div className="row">
        <span>
          <i style={{ '--c': 'var(--chart-reserve)' } as React.CSSProperties} />
          Required minimum
        </span>
      ...
      {flow !== 0 && (
        <div className="row">
          <span>
            <i style={{ '--c': flow > 0 ? 'var(--chart-charge)' : 'var(--chart-solar)' } as React.CSSProperties} />
      ...
      <div className="card-head">
        <div>
          ...
          <div className="small muted">
            ...
            {margin >= -0.011 ? 'never below the required reserve' : <span style={{ color: 'var(--chart-reserve)' }}>breaches the required reserve</span>}
          </div>
        </div>
        ...
        <ComposedChart data={data} margin={{ top: 10, right: 6, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id="battFill" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="var(--chart-battery-fill-top)" />
              <stop offset="100%" stopColor="var(--chart-battery-fill-bottom)" />
            </linearGradient>
          </defs>
          ...
          <ReferenceLine yAxisId="kwh" y={b.capacity_kwh} stroke="var(--chart-axis)" strokeOpacity={0.5} strokeDasharray="3 4" label={{ value: 'capacity', ...AXIS, position: 'insideTopRight' }} />
          <ReferenceLine yAxisId="kwh" y={b.initial_energy_kwh} stroke="var(--chart-axis)" strokeOpacity={0.5} strokeDasharray="2 5" label={{ value: 'start & end level', ...AXIS, position: 'insideBottomRight' }} />
          <Area yAxisId="kwh" type="stepAfter" dataKey="reserve" stroke="var(--chart-reserve)" strokeDasharray="4 3" strokeWidth={1.5} fill="var(--chart-reserve-fill)" isAnimationActive={false} />
          <Area yAxisId="kwh" type="monotone" dataKey="energy" stroke="var(--chart-battery)" strokeWidth={2.4} fill="url(#battFill)" dot={{ r: 2.5, fill: 'var(--chart-battery)', strokeWidth: 0 }} isAnimationActive={false} />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
  )
